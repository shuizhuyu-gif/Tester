...
class Testlogin(unittest.TestCase):
    ...
    def setUpClass(cls):  #测试类，执行一次
        cls.data_list = excel_data_list(os.path.join(data_path,"test_data.xlsx"),"test_reg")
        #data_list 同 self.data_list 都是该类的公共属性

    def test_add_normal(self):
        case_data = get_test_data(self.data_list,"test_add_normal")
        if not case_data:
            ...
            log_case_info("test_add_normal", url, data, expect_data, requested.text)
            self.assertEqual(requested.text, expect_data)  # 断言

    def test_select_normal(self):
        case_data = get_test_data(self.data_list,"test_select_normal")   #在数据列表中查找用例数据
        if not case_data:
            logging.error("未找到该用例数据")
        #数据对应
        ...
        requested = requests.post(url=url, data=json.loads(data))
        log_case_info("test_select_normal", url, data, expect_data, requested.text)
        self.assertEqual(requested.text, expect_data)  # 断言
